...
// Cliente Supabase direto (compatível com qualquer versão)
...
type StatusGeral = {
  ...
  proximo_sync: { empresa: string; minuto_da_hora: number; em_minutos: number } | null
}
...
function Badge({ fase }: { fase: string }) {
  ...
  return (
    <span style={{ display: 'inline-block', padding: '4px 10px', borderRadius: 999, fontSize: 12, fontWeight: 600, backgroundColor: c.bg, color: c.fg, letterSpacing: 0.2 }}>
      {c.label}
    </span>
  )
}
...
export default function SyncStatusPage() {
  ...
  const executarAcao = async (acao: string, companyId?: string) => {
    ...
    if (error) { setMensagemAcao(`Erro: ${error.message}`); return }
  ...
  return (
    <main style={{ minHeight: '100vh', backgroundColor: '#FAF7F2', fontFamily: 'Inter, system-ui, sans-serif', color: '#3D2314', padding: '32px 24px 64px' }}>
      <div style={{ maxWidth: 1280, margin: '0 auto' }}>
        ...
        {statusGeral && (
          <section style={{ marginBottom: 40 }}>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16 }}>
              <Card titulo="Crons ativos" valor={`${statusGeral.crons_ativos} / 2`} destaque={statusGeral.crons_ativos === 2 ? 'ok' : 'alerta'} />
              <Card titulo="Empresas agendadas" valor={String(statusGeral.empresas_agendadas)} />
              <Card titulo="Syncs nas últimas 24h" valor={String(statusGeral.syncs_ultimas_24h)} detalhe={`${statusGeral.syncs_sucesso_24h} sucesso · ${statusGeral.syncs_falha_24h} falha`} />
              <Card titulo="Em andamento" valor={String(statusGeral.syncs_em_andamento)} destaque={statusGeral.syncs_em_andamento > 0 ? 'ativo' : undefined} />
              {statusGeral.proximo_sync && (
                <Card titulo="Próximo sync" valor={statusGeral.proximo_sync.empresa.split(' ')[0]} detalhe={`em ${statusGeral.proximo_sync.em_minutos} min (minuto ${statusGeral.proximo_sync.minuto_da_hora})`} />
              )}
            </div>
          </section>
        )}
        ...
        <section style={{ marginBottom: 40 }}>
          <h2 style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: 24, fontWeight: 400, margin: '0 0 16px' }}>
            Por empresa
          </h2>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
            {resumo.map((e) => (
              <div key={e.company_id} style={{ backgroundColor: 'white', borderRadius: 12, padding: 20, boxShadow: '0 1px 3px rgba(61, 35, 20, 0.06)' }}>
                ...
                {e.ultimo_sync_fase && (
                  <div style={{ marginBottom: 12 }}>
                    ...
                    <span style={{ marginLeft: 8, fontSize: 13, opacity: 0.7 }}>{e.ultimo_sync_local || '—'}</span>
                  </div>
                )}
        ...
        <section style={{ marginBottom: 40 }}>
          ...
          <div style={{ backgroundColor: 'white', borderRadius: 12, padding: 20, boxShadow: '0 1px 3px rgba(61, 35, 20, 0.06)', display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <section>
          ...
          <div style={{ backgroundColor: 'white', borderRadius: 12, overflow: 'hidden', boxShadow: '0 1px 3px rgba(61, 35, 20, 0.06)' }}>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                <thead>
                  <tr style={{ backgroundColor: '#f5f0e8' }}>
                    <Th>Empresa</Th><Th>Início</Th><Th>Duração</Th><Th>Status</Th><Th>Gatilho</Th><Th>A Pagar / Receber</Th><Th>Clientes</Th><Th>Erro</Th>
                  </tr>
                </thead>
                <tbody>
                  {ultimos.map((l, i) => (
                    <tr key={l.id} style={{ borderTop: i === 0 ? 'none' : '1px solid #f0ebe0' }}>
                      <Td><span style={{ fontWeight: 500 }}>{l.empresa.split(' ')[0]}</span></Td>
                      <Td mono>{l.iniciado_local}</Td>
                      <Td mono>{l.duracao_s != null ? `${l.duracao_s}s` : '—'}</Td>
                      <Td><Badge fase={l.fase} /></Td>
                      <Td><span style={{ fontSize: 11, opacity: 0.7 }}>{l.trigger_type}</span></Td>
                      <Td mono>
                        {l.sync_counts ? `${l.sync_counts.contas_pagar ?? 0} / ${l.sync_counts.contas_receber ?? 0}` : '—'}
                      </Td>
                      <Td mono>{l.sync_counts?.clientes ?? '—'}</Td>
                      <Td>
                        {l.erro && (
                          <span style={{ fontSize: 12, color: '#a02020', maxWidth: 200, display: 'inline-block', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }} title={l.erro}>
                            {l.erro}
                          </span>
                        )}
                      </Td>
            ...
            {ultimos.length === 0 && (
              <p style={{ padding: 24, textAlign: 'center', opacity: 0.5, fontSize: 13 }}>Nenhum sync registrado ainda.</p>
            )}
          </div>
        </section>
      </div>
...
function Card({ titulo, valor, detalhe, destaque }: { titulo: string; valor: string; detalhe?: string; destaque?: 'ok' | 'alerta' | 'ativo' }) {
  const corValor = destaque === 'ok' ? '#2d6a3e' : destaque === 'alerta' ? '#a02020' : destaque === 'ativo' ? '#C8941A' : '#3D2314'
  return (
    <div style={{ backgroundColor: 'white', borderRadius: 12, padding: 20, boxShadow: '0 1px 3px rgba(61, 35, 20, 0.06)' }}>
      <p style={{ fontSize: 11, fontWeight: 600, letterSpacing: 1.5, textTransform: 'uppercase', opacity: 0.5, margin: 0 }}>{titulo}</p>
      <p style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: 32, fontWeight: 400, margin: '6px 0 0', color: corValor, letterSpacing: -0.3 }}>{valor}</p>
      {detalhe && <p style={{ fontSize: 12, opacity: 0.65, margin: '4px 0 0' }}>{detalhe}</p>}
    </div>
  )
}

function Th({ children }: { children: React.ReactNode }) {
  return <th style={{ padding: '12px 16px', textAlign: 'left', fontSize: 11, fontWeight: 600, letterSpacing: 1, textTransform: 'uppercase', opacity: 0.6 }}>{children}</th>
}

function Td({ children, mono }: { children: React.ReactNode; mono?: boolean }) {
  return <td style={{ padding: '12px 16px', fontSize: 13, fontFamily: mono ? 'JetBrains Mono, Menlo, monospace' : undefined, color: '#3D2314' }}>{children}</td>
}
