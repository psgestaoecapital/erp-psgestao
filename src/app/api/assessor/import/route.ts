...
export async function POST(req: NextRequest) {
  ...
  if (dados_csv && tipo_abc) {
    ...
    await supabase.from('diagnostico_imports').insert({ diagnostico_id, tipo_arquivo: 'csv', nome_arquivo: tipo_abc + '_import.csv', dados_raw: rows, dados_parseados: abc.dados, status: 'processado' });
    const { error: upsertErr } = await supabase.from('diagnostico_curvas_abc').insert({ diagnostico_id, tipo: tipo_abc, dados: abc.dados, total_itens: abc.total_itens, total_valor: abc.total_valor, qtd_a: abc.qtd_a, qtd_b: abc.qtd_b, qtd_c: abc.qtd_c });
    ...
    return NextResponse.json({ message: 'Curva ABC gerada', abc: { total_itens: abc.total_itens, qtd_a: abc.qtd_a, qtd_b: abc.qtd_b, qtd_c: abc.qtd_c, total_valor: abc.total_valor } });
  }
