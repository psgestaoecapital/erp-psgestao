...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Documento tem que ser CPF (11) ou CNPJ (14). Fora disso o banco recusa (422) por
    // divergência com o tipo. Barra ANTES de ir ao banco, dizendo o que falta e o impacto.
    ...
    if (result.status < 200 || result.status >= 300 || !result.nuTituloGerado || !result.linhaDigitavel || !result.codigoBarras) {
      ...
      const erro = msgBanco ? `Sicredi nao registrou o boleto: ${msgBanco}` : 'Sicredi recusou o registro do boleto.'
