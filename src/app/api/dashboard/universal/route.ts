// Dashboard Universal — serve 5 planos (Comércio, Industrial, Agro, BPO, P&M)
// Aceita company_id, company_ids ou grupo_id; período configurável
  ...
  try {
    ...
    if (grupoId) {
      ...
      const { data: grupo } = await supabase
        .from('dashboard_grupos').select('nome').eq('id', grupoId).maybeSingle();
      ...
      const { data: grupoPadrao } = await supabase
        ...
        .eq('user_id', user.userId).eq('is_padrao', true).maybeSingle();
      if (grupoPadrao) {
    ...
    if (companyIds.length === 0) {
      return NextResponse.json({ erro: 'Nenhuma empresa selecionada', requer_config: true }, { status: 200 });
    }

    const { ano, mes, dataInicio, dataFim } = resolverPeriodo(periodo, anoParam, mesParam);

    const [saudeRes, dreRes, topClientesRes, topFornRes, atalhosData, fluxoRes] = await Promise.all([
      supabase.rpc('fn_psgc_saude_consolidada', { p_company_ids: companyIds, p_ano: ano, p_mes: mes }),
      supabase.rpc('fn_psgc_dre_consolidada', { p_company_ids: companyIds, p_ano: ano, p_mes: mes }),
      supabase.rpc('fn_psgc_abc_consolidado', { p_company_ids: companyIds, p_ano: ano, p_tipo: 'cliente', p_limit: 5 }),
      supabase.rpc('fn_psgc_abc_consolidado', { p_company_ids: companyIds, p_ano: ano, p_tipo: 'fornecedor', p_limit: 5 }),
      buscarAtalhos(supabase, user.userId, plano),
      supabase.from('psgc_fluxo_realizado').select('data, entradas, saidas, saldo_dia')
        .in('company_id', companyIds).gte('data', dataInicio).lte('data', dataFim).order('data')
    ]);
    ...
    return NextResponse.json({
      contexto: { plano, nome: nomeContexto, company_ids: companyIds, qtd_empresas: companyIds.length, periodo, ano, mes },
      camada1: {
        saude: {
        ...
        futuro: {
          fluxo: fluxoRes.data || [],
          saldo_projetado_60d: (fluxoRes.data || []).reduce((sum: number, d: any) => sum + Number(d.saldo_dia || 0), 0)
        }
      },
      camada2: {
    ...
    console.error('[dashboard/universal]', error);
...
function resolverPeriodo(periodo: string, anoParam: string | null, mesParam: string | null) {
  ...
  switch (periodo) {
    case 'hoje':
      dataInicio = dataFim = hoje.toISOString().split('T')[0]; break;
    case 'sem':
      const inicioSem = new Date(hoje.getTime() - hoje.getDay() * 86400000);
      dataInicio = inicioSem.toISOString().split('T')[0];
      dataFim = hoje.toISOString().split('T')[0]; break;
    case 'tri':
      const tri = Math.floor(hoje.getMonth() / 3);
      dataInicio = new Date(ano, tri * 3, 1).toISOString().split('T')[0];
      dataFim = new Date(ano, tri * 3 + 3, 0).toISOString().split('T')[0]; break;
    case '6m':
      dataInicio = new Date(hoje.getTime() - 180 * 86400000).toISOString().split('T')[0];
      dataFim = hoje.toISOString().split('T')[0]; break;
    case 'ano':
      dataInicio = `${ano}-01-01`; dataFim = `${ano}-12-31`; break;
    case 'mes':
    default:
      dataInicio = new Date(ano, mes - 1, 1).toISOString().split('T')[0];
      dataFim = new Date(ano, mes, 0).toISOString().split('T')[0]; break;
  }
  return { ano, mes, dataInicio, dataFim };
}

async function buscarAtalhos(supabase: any, userId: string, plano: string) {
  const { data: custom } = await supabase.from('dashboard_atalhos').select('*')
    .eq('user_id', userId).eq('plano', plano).order('ordem');
  if (custom && custom.length > 0) return custom;
  const { data: defaults } = await supabase.from('dashboard_atalhos_default').select('*')
    .eq('plano', plano).order('ordem');
  return defaults || [];
}
...
async function buscarAcoes(supabase: any, companyIds: string[]) {
  ...
  const [boletos, recebiveis] = await Promise.all([
    supabase.from('erp_pagar').select('id, fornecedor_nome, valor, data_vencimento, company_id')
      .in('company_id', companyIds).gte('data_vencimento', hoje).lte('data_vencimento', em3Dias)
      .neq('status', 'pago').order('data_vencimento').limit(20),
    supabase.from('erp_receber').select('id, cliente_nome, valor, data_vencimento, company_id')
      .in('company_id', companyIds).lt('data_vencimento', hoje)
      .neq('status', 'pago').order('data_vencimento').limit(20)
  ]);
  const acoes: any[] = [];
