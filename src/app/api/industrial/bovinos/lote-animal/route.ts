...
export const POST = withAuth(async (req: NextRequest, { user }) => {
  ...
  const custo_kg_vivo = custo_arroba ? custo_arroba / 15 : null // 1@ = 15kg
  const custo_total_lote = custo_arroba && quantidade && peso_medio_entrada
    ? quantidade * peso_medio_entrada * (custo_arroba / 15)
    : null

  const { data, error } = await supabaseAdmin
    .from('ind_lotes_animais')
    .insert({ unidade_id, data_entrada, fornecedor, tipo_aquisicao: tipo_aquisicao||'compra',
              quantidade, peso_medio_entrada, custo_arroba, custo_kg_vivo,
              custo_frete_cab: custo_frete_cab||0, custo_total_lote })
    .select().single()
...
export const GET = withAuth(async (req: NextRequest, { user }) => {
  ...
  const { data } = await supabaseAdmin
    ...
    .eq('unidade_id', unidadeId).order('data_entrada', { ascending: false }).limit(30)
