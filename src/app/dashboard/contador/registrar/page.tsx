...
export default function RegistrarContadorPage() {
  ...
  const [form, setForm] = useState({
    nome: '', cnpj: '', responsavel: '', email: '', telefone: '', crc_uf: ''
  })
    ...
    <div className="p-6 max-w-lg mx-auto">
      ...
      <div className="bg-amber-50 border border-amber-300 rounded-lg p-4 mb-4">
        <p className="text-sm font-bold text-amber-800 mb-2">
          ⚠ Guarde sua API Key — não será exibida novamente:
        </p>
        <code className="text-xs break-all bg-white p-2 rounded block border select-all">
          {apiKey}
        </code>
      </div>
      <p className="text-xs text-muted-foreground mb-4">
        Use esta chave para acessar a API do contador via header:<br/>
        Authorization: Bearer {apiKey}
      </p>
      <button
        onClick={() => router.push('/dashboard/contador')}
  ...
  return (
    <div className="p-6 max-w-lg mx-auto">
      ...
      <p className="text-sm text-muted-foreground mb-6">
        ...
        gerar SPED e simular o impacto da Reforma Tributária.
      </p>

      {error && (
        <div className="bg-red-50 border border-red-300 rounded p-3 text-sm text-red-800 mb-4">
          {error}
        </div>
      )}
      ...
      <div className="space-y-3">
        {[
          ...
          ['responsavel', 'Responsável (CRC)',   'text',  false],
