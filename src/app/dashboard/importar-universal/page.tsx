    ...
    <div style={{minHeight:"100vh",background:C.bg,color:C.tx,fontFamily:"'Segoe UI',system-ui,sans-serif"}}>
      <div style={{background:C.esp,padding:"12px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:`2px solid ${C.go}`}}>
        <div>
          ...
          <div style={{fontSize:10,color:C.txm}}>Aceita Excel, CSV de qualquer sistema · Detecção automática de tipo e colunas</div>
        </div>
      ...
      {step==="upload"&&(<>
        ...
        <div style={{background:C.bg2,borderRadius:12,padding:16,border:`1px solid ${C.bd}`}}>
          ...
          <div style={{marginTop:12,padding:10,background:C.bg3,borderRadius:8}}>
            <div style={{fontSize:10,fontWeight:600,color:C.gol,marginBottom:4}}>💡 Dica: O sistema detecta automaticamente!</div>
            <div style={{fontSize:9,color:C.txm}}>Exporte do seu sistema atual (Nedel, Bling, ContaAzul, planilha manual, etc.) em Excel ou CSV. Nosso motor de IA identifica as colunas automaticamente. Não precisa ajustar nada — é só arrastar e importar.</div>
          </div>
        </div>
      </>)}
      ...
      {step==="preview"&&analysis&&(<>
        ...
        <div style={{background:C.bg2,borderRadius:12,padding:14,border:`1px solid ${C.bd}`,marginBottom:12}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            <div>
              ...
              <div style={{fontSize:9,color:C.txd}}>{(analysis.fileSize/1024).toFixed(0)} KB · {analysis.totalRows} linhas detectadas · {analysis.headers.length} colunas</div>
            </div>
          ...
          <div style={{fontSize:11,fontWeight:600,color:C.gol,marginBottom:6}}>🔗 Mapeamento de Colunas (automático)</div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:4,marginBottom:8}}>
            {Object.entries(analysis.mapping).map(([field,colIdx])=>(
              <div key={field} style={{display:"flex",justifyContent:"space-between",padding:"3px 6px",background:C.bg3,borderRadius:4,borderLeft:`2px solid ${C.g}`}}>
                ...
                <span style={{fontSize:9,color:C.txm}}>← {analysis.headers[colIdx as number]}</span>
              </div>
            ))}
          </div>
          {analysis.unmappedHeaders?.length>0&&(
            <div style={{fontSize:9,color:C.txd}}>Colunas não mapeadas: {analysis.unmappedHeaders.join(", ")}</div>
          )}
        </div>
        ...
        <div style={{display:"flex",gap:8,justifyContent:"center"}}>
          <button onClick={doImport} disabled={loading} style={{padding:"12px 32px",borderRadius:10,border:"none",background:tipoInf?.cor||C.go,color:C.bg,fontSize:14,fontWeight:700,cursor:loading?"wait":"pointer",opacity:loading?0.5:1}}>
            {loading?"⏳ Importando...":`${tipoInf?.icon||"📤"} Importar ${analysis.totalRows} registros como ${tipoInf?.nome||tipo}`}
          </button>
      ...
      {step==="done"&&result&&(
        <div style={{background:C.bg2,borderRadius:16,padding:30,border:`1px solid ${C.g}40`,textAlign:"center"}}>
          ...
          <div style={{display:"flex",gap:8,justifyContent:"center"}}>
            <button onClick={reset} style={{padding:"10px 24px",borderRadius:8,border:"none",background:C.go,color:C.bg,fontSize:12,fontWeight:700,cursor:"pointer"}}>📤 Importar outro arquivo</button>
            <a href="/dashboard/operacional" style={{padding:"10px 24px",borderRadius:8,border:`1px solid ${C.bd}`,color:C.gol,fontSize:12,fontWeight:600,textDecoration:"none"}}>📋 Ver no Operacional</a>
            <a href="/dashboard" style={{padding:"10px 24px",borderRadius:8,border:`1px solid ${C.bd}`,color:C.txm,fontSize:12,textDecoration:"none"}}>← Dashboard</a>
          </div>
        </div>
      )}
