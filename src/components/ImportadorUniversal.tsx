...
const COLUMN_ALIASES: Record<string, string[]> = {
  nome_pessoa: ["nome fantasia","nome fantasia/apelido","razao social","razão social","fornecedor","cliente","nome","favorecido","pagador","sacado","nome_pessoa","nome_fantasia","razao_social","beneficiario","beneficiário","parceiro","contato","empresa","name"],
  descricao: ["descricao","descrição","descr","historico","histórico","observacao","observação","obs","memo","detalhes","description","complemento"],
  valor_documento: ["valor","valor base","valor_documento","vlr","total","montante","amount","valor total","valor doc","valor_base"],
  valor_receber: ["receber","receita","entrada","credito","crédito","credit","valor receber","valor_receber","income","revenue"],
  valor_pagar: ["pagar","despesa","saida","saída","debito","débito","debit","valor pagar","valor_pagar","expense","pagamento"],
  data_emissao: ["emissao","emissão","data emissao","data emissão","dt emissao","data_emissao","issue date","data lancamento","data lançamento","competencia","competência"],
  data_vencimento: ["vencimento","data vencimento","dt vencimento","data_vencimento","due date","prazo","validade","venc"],
  data_pagamento: ["quitado","quitado em","data pagamento","dt pagamento","data_pagamento","pago em","liquidado em","data quitacao","data quitação","payment date"],
  status: ["situacao","situação","status","estado","status_titulo","sit"],
  categoria: ["categoria","category","plano de contas","plano_contas","conta","classificacao","classificação","tipo despesa"],
  centro_custo: ["centro de custos","centro de custo","centro_custo","cc","departamento","setor","cost center"],
  numero_documento: ["documento","doc","numero","número","nf","nota","nota fiscal","numero_documento","cod","codigo","código","nfe","num doc"],
  conta_corrente: ["conta corrente","conta_corrente","banco","bank","conta bancaria","conta bancária"],
};
...
function parseDate(val: string): string | null {
  ...
  if (p1) { let a = parseInt(p1[3]); if (p1[3].length === 2) a += 2000; return `${a}-${String(parseInt(p1[2])).padStart(2,"0")}-${String(parseInt(p1[1])).padStart(2,"0")}`; }
...
interface ImporterProps { companyId: string; onImportComplete?: (count: number) => void; }
...
export default function ImportadorUniversal({ companyId, onImportComplete }: ImporterProps) {
      ...
      if (ext === "csv" || ext === "txt" || ext === "tsv") {
        ...
        for (const enc of ["UTF-8", "ISO-8859-1", "Windows-1252"]) {
          try {
            ...
            if (enc !== "UTF-8") { const buf = await file.arrayBuffer(); const decoder = new TextDecoder(enc); text = decoder.decode(buf); }
        ...
        setErrorMsg("Para arquivos Excel, exporte como CSV primeiro. Suporte XLSX em breve.");
      } else { setErrorMsg("Formato não suportado. Use CSV, TXT ou TSV."); }
    } catch (err: any) { setErrorMsg(`Erro ao ler arquivo: ${err.message}`); }
  }, []);
  ...
  const getMappedRows = () => {
    return rawRows.filter(row => {
      ...
      let subcategoria = "";
      if (catRaw.includes(" > ")) { const parts = catRaw.split(" > ", 2); categoria = parts[0].trim(); subcategoria = parts[1].trim(); }
      return {
        ...
        observacao_interna: get("conta_corrente")?.replace(/^[•\s]+/, "").trim() || "",
      };
  ...
  const doImport = async () => {
    ...
    for (let i = 0; i < rows.length; i += batchSize) {
      ...
      if (error) { console.error("Erro batch:", error); errors += batch.length; }
  ...
  return (
    <div style={{ minHeight: "100vh", background: BG, padding: 20 }}>
      ...
      {errorMsg && (<div style={{ ...card, borderColor: R + "60", background: R + "10" }}><div style={{ color: R, fontSize: 12 }}>⚠️ {errorMsg}</div></div>)}
      {step === "upload" && (
        <div style={{ ...card, textAlign: "center", padding: 40 }}>
          ...
          <div style={{ fontSize: 10, color: TXD, marginTop: 16 }}>O sistema detecta automaticamente: separador, formato de número, formato de data e encoding</div>
        </div>
      )}
      {step === "mapping" && (
        <>
          <div style={card}>
            <div style={{ fontSize: 14, fontWeight: 600, color: GOL, marginBottom: 12 }}>🔗 Mapeamento de colunas — {fileName}<span style={{ fontSize: 11, color: TXD, fontWeight: 400, marginLeft: 8 }}>{rawRows.length} linhas, {headers.length} colunas</span></div>
            <div style={{ fontSize: 11, color: TXM, marginBottom: 16 }}>O sistema mapeou automaticamente as colunas. Ajuste se necessário.</div>
              ...
              {Object.entries(COLUMN_ALIASES).map(([field]) => {
                ...
                return (
                  <React.Fragment key={field}>
                    <div style={{ fontSize: 11, color: isRequired ? TX : TXM }}>{field.replace(/_/g, " ")}{isRequired && " *"}</div>
                    <select value={idx ?? -1} onChange={e => { const v = parseInt(e.target.value); setMapping(prev => ({ ...prev, [field]: v === -1 ? null : v })); }} style={sel}>
                      <option value={-1}>— não mapear —</option>
                      {headers.map((h, i) => <option key={i} value={i}>{h}</option>)}
                    </select>
                    <div style={{ fontSize: 10, color: hasValue ? G : TXD }}>{hasValue ? `✓ ${detected || headers[idx!]}` : "—"}</div>
                  </React.Fragment>
                );
              })}
            </div>
      ...
      {step === "preview" && (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8, marginBottom: 12 }}>
            {[{l:"Total registros",v:mapped.length,c:GOL},{l:"A receber",v:fmtR(totalReceber),c:G},{l:"A pagar",v:fmtR(totalPagar),c:R},{l:"Líquido",v:fmtR(totalReceber-totalPagar),c:totalReceber-totalPagar>=0?G:R}].map((m,i) => (
          ...
          <div style={{ ...card, padding: 0, overflow: "hidden" }}>
            <div style={{ padding: "10px 16px", borderBottom: `1px solid ${BD}`, fontSize: 13, fontWeight: 600, color: GOL }}>Preview (primeiros 20 registros)</div>
                ...
                <tbody>
                  {mapped.slice(0, 20).map((r, i) => (
                    <tr key={i} style={{ borderBottom: `0.5px solid ${BD}30` }}>
                      <td style={{ padding: "6px", color: r.tipo === "receber" ? G : R, fontWeight: 600 }}>{r.tipo === "receber" ? "↓ REC" : "↑ PAG"}</td>
                      ...
                      <td style={{ padding: "6px", textAlign: "center" }}><span style={{ fontSize: 9, padding: "2px 6px", borderRadius: 4, background: r.status === "PAGO" || r.status === "RECEBIDO" ? G + "20" : r.status === "PENDENTE" ? B + "20" : Y + "20", color: r.status === "PAGO" || r.status === "RECEBIDO" ? G : r.status === "PENDENTE" ? B : Y }}>{r.status}</span></td>
            ...
            {mapped.length > 20 && (<div style={{ padding: "8px 16px", textAlign: "center", fontSize: 10, color: TXD, borderTop: `1px solid ${BD}30` }}>+{mapped.length - 20} registros não mostrados</div>)}
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button style={btnSec} onClick={() => setStep("mapping")}>← Ajustar mapeamento</button>
            <button style={btn} onClick={doImport}>✅ Importar {mapped.length} registros</button>
          </div>
        </>
      )}
      {step === "importing" && (
        <div style={{ ...card, textAlign: "center", padding: 40 }}>
          ...
          <div style={{ width: "100%", height: 8, background: BG3, borderRadius: 4, overflow: "hidden", marginBottom: 8 }}><div style={{ width: `${progress}%`, height: "100%", background: GO, transition: "width 0.3s" }} /></div>
          <div style={{ fontSize: 11, color: TXD }}>{progress}% concluído</div>
        </div>
      )}
      {step === "done" && (
        <div style={card}>
          <div style={{ fontSize: 16, fontWeight: 700, color: importResult.errors === 0 ? G : Y, marginBottom: 12 }}>{importResult.errors === 0 ? "✅ Importação concluída!" : "⚠️ Importação parcial"}</div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginBottom: 16 }}>
            <div style={{ background: BG3, borderRadius: 8, padding: 12, textAlign: "center" }}><div style={{ fontSize: 22, fontWeight: 700, color: G }}>{importResult.success}</div><div style={{ fontSize: 10, color: TXD }}>Importados</div></div>
            <div style={{ background: BG3, borderRadius: 8, padding: 12, textAlign: "center" }}><div style={{ fontSize: 22, fontWeight: 700, color: importResult.errors > 0 ? R : TXD }}>{importResult.errors}</div><div style={{ fontSize: 10, color: TXD }}>Erros</div></div>
            <div style={{ background: BG3, borderRadius: 8, padding: 12, textAlign: "center" }}><div style={{ fontSize: 22, fontWeight: 700, color: GOL }}>{importResult.total}</div><div style={{ fontSize: 10, color: TXD }}>Total</div></div>
          </div>
          <div style={{ display: "flex", gap: 8 }}>
            <button style={btn} onClick={() => { setStep("upload"); setRawRows([]); setHeaders([]); setProgress(0); }}>Importar outro arquivo</button>
            <a href="/dashboard/visao-mensal?empresa=consolidado" style={{ ...btnSec, textDecoration: "none", display: "inline-block" }}>Ver no Dashboard →</a>
          </div>
        </div>
      )}
      <div style={{ fontSize: 9, color: TXD, textAlign: "center", marginTop: 16 }}>PS Gestão e Capital — Importador Universal v1.1</div>
    </div>
  );
}
