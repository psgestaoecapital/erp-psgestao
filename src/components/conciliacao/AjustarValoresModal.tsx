...
export default function AjustarValoresModal({
  ...
  useEffect(() => {
    if (open) {
      const diff = (valorBanco ?? 0) - valorOriginal
      if (diff > 0) {
        setJuros(diff.toFixed(2))
        setDesconto('')
      } else if (diff < 0) {
        setDesconto(Math.abs(diff).toFixed(2))
        setJuros('')
      } else {
        setJuros('')
        setDesconto('')
    ...
    <div onClick={onClose} style={overlay}>
      <div onClick={(e) => e.stopPropagation()} style={modal}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
          ...
          {valorBanco != null && (
            <Info label="Valor banco" valor={`R$ ${fmtBRL(valorBanco)}`} cor="#3D2314" />
          )}
        </div>
        ...
        {valorBanco != null && diferenca !== 0 && (
          <div style={{ background: '#FAEEDA', border: '0.5px solid rgba(186,117,23,0.3)', padding: '8px 12px', borderRadius: 6, marginBottom: 12, fontSize: 12, color: '#854F0B' }}>
            ...
            {diferenca > 0 ? ' · sugerimos lançar como juros' : ' · sugerimos lançar como desconto'}
          </div>
        )}
        ...
        <div style={{ background: '#EAF3DE', border: '0.5px solid rgba(59,109,17,0.25)', padding: '10px 14px', borderRadius: 6, marginTop: 12, fontSize: 13, color: '#3B6D11' }}>
          Valor ajustado final: <strong>R$ {fmtBRL(valorAjustado)}</strong>
        </div>
