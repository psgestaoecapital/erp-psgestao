...
// Editor COMPLETO de erp_pagar / erp_receber (F2). Expõe TODOS os campos editáveis do lançamento,
// não só os essenciais. Salva via fn_pagar_editar_completo / fn_receber_editar_completo (payload jsonb),
// que gravam a trilha em erp_lancamento_log (só o que mudou, de→para). RLS garante isolamento por company_id.
//
// Aviso (NÃO bloqueia): se o lançamento está PAGO ou CONCILIADO e o usuário mexe num campo financeiro
// (valor, data_pagamento, conta_bancaria), mostra alerta — editar aqui não altera baixa/vínculo bancário.
// Editar campo não-financeiro (descrição, categoria, obs…) num pago/conciliado NÃO dispara aviso.
//
// Campos de sistema (id, company_id, datas de auditoria, import_hash, boleto_*, conciliado, status, valor_pago)
// NÃO aparecem aqui — têm fluxos próprios (marcar pago / conciliação). O whitelist real vive na RPC.
...
const CAMPOS_FINANCEIROS = ['valor', 'data_pagamento', 'conta_bancaria']

// Definição dos campos editáveis. `col` = coluna real (= chave do payload/whitelist da RPC).
type Campo = { col: string; label: string; tipo: 'text' | 'num' | 'date' | 'area' | 'bool' | 'select'; opcoes?: string[]; largo?: boolean }
...
function campos(tipo: Tipo): Campo[] {
  ...
  if (tipo === 'pagar') {
    base.splice(11, 0, { col: 'codigo_barras', label: 'Código de barras', tipo: 'text', largo: true })
  } else {
    base.splice(11, 0, { col: 'contrato_id', label: 'Contrato (UUID)', tipo: 'text', largo: true })
  }
  return base
}
...
export default function EditarLancamentoModal({
  ...
  // Aviso só quando pago/conciliado E algum campo FINANCEIRO foi de fato alterado.
  ...
  async function salvar() {
    ...
    try {
      // Envia só os campos que mudaram — a RPC recalcula o diff e grava a trilha de qualquer jeito,
      // mas mandar só o alterado deixa o UPDATE enxuto.
  ...
  return (
    <div role="dialog" aria-modal="true" style={{
      ...
      <div onClick={(e) => e.stopPropagation()} style={{
        background: BG, borderRadius: 12, maxWidth: 720, width: '100%',
        ...
        <div style={{ padding: '16px 20px', borderBottom: `0.5px solid ${LINE}` }}>
          <div style={{ fontSize: 11, color: ESP60, textTransform: 'uppercase', letterSpacing: 1 }}>
            Editar {tipo === 'pagar' ? 'conta a pagar' : 'conta a receber'}
          </div>
          <div style={{ fontSize: 16, color: ESP, fontWeight: 600, marginTop: 4 }}>
            {form.descricao || (carregando ? 'Carregando…' : '—')}
          </div>
        </div>
              ...
              return (
                <div key={c.col} style={span}>
                  ...
                  {c.tipo === 'area' ? (
                    ...
                    <input
                      value={val}
                      onChange={(e) => set(c.col, e.target.value)}
