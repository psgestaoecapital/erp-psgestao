...
export default function ImportMigracaoFinanceiraCard({ companyId, empresaNome }: { companyId: string; empresaNome?: string | null }) {
  ...
  const cont = useMemo(() => {
    let ok = 0, aviso = 0, erro = 0, pagar = 0, receber = 0, total = 0
    ...
    return { ok, aviso, erro, pagar, receber, total }
  }, [linhas])
  ...
  async function onArquivo(file: File) {
    ...
    try {
      ...
      for (let i = idxHeader + 1; i < matriz.length; i++) {
        ...
        if (EXEMPLOS.has(`${tipo}|${valorRaw}|${vencRaw}`)) avisos.push('Linha de exemplo do modelo — confira/apague antes de importar')
        ...
        parsed.push({
          ...
          nivel: erros.length ? 'erro' : avisos.length ? 'aviso' : 'ok', msgs: [...erros, ...avisos],
        })
      }
  ...
  const podeImportar = linhas.length > 0 && !importando && (cont.ok + cont.aviso) > 0
      ...
      {linhas.length > 0 && (
        <>
          <div className="flex items-center justify-between flex-wrap gap-2 text-xs" style={{ color: ESP60 }}>
            <span>🟢 {cont.ok} ok · 🟡 {cont.aviso} aviso(s) · 🔴 {cont.erro} erro(s) · {cont.pagar} a pagar · {cont.receber} a receber · total {fmtBRL(cont.total)}</span>
          </div>
          ...
          <button type="button" onClick={() => void importar()} disabled={!podeImportar}
            ...
            {importando ? 'Importando…' : `Importar ${cont.ok + cont.aviso} lançamento(s) para ${empresaNome ?? 'a empresa'}`}
          </button>
        </>
      )}
