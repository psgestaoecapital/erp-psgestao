...
// cadastro-produtos-importar-fiscal-v1
// Modal que reusa ImportProdutosFiscalCard (mesma RPC fn_import_produtos_fiscal)
// e dispara onAtualizado() depois que o usuario aplica, pra recarregar a lista.

import { useEffect } from 'react'
import { X } from 'lucide-react'
import ImportProdutosFiscalCard from './ImportProdutosFiscalCard'
...
export default function ImportProdutosFiscalModal({ companyId, onClose, onAtualizado }: Props) {
  useEffect(() => {
      ...
      >
        <div style={{
          ...
          <h2 style={{ fontSize: 16, fontWeight: 600, color: '#C8941A', margin: 0, fontFamily: 'Fraunces, Georgia, serif' }}>
            🧾 Importar planilha fiscal de produtos
          </h2>
          <button
            onClick={onClose}
            aria-label="Fechar"
        ...
        <div style={{ padding: 16 }}>
          <ImportProdutosFiscalCard companyId={companyId} onAplicado={onAtualizado} />
        </div>
      </div>
    </div>
  )
