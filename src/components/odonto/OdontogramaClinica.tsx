'use client'
// RD-41 · Odonto OD-2 — Builder clínico REUTILIZÁVEL (odontograma + plano com valor + concluir +
// aprovar). Extraído da tela /clinica (que tinha "Selecione um paciente"): aqui o paciente vem por
// PROP (a Ficha já é por paciente — fim do select-patient · feedback do CEO). Lógica idêntica à
// tela original (RD-55: nada de dinheiro reescrito; reusa fn_odonto_plano_salvar / _item_concluir /
// _plano_aprovar_financeiro / _odontograma_estado). Usado em Orçamentos (montar) e Tratamentos (executar).
import { useCallback, useEffect, useMemo, useState } from 'react'
import { supabase } from '@/lib/supabase'
import { CardOdonto, TOK } from './ui'
import { Odontograma, grupoDentes, FACES, FACE_LABEL, type Face } from './Odontograma'
...
export function OdontogramaClinica({ companyId, pacienteId, onMudou }: { companyId: string; pacienteId: string; onMudou?: () => void }) {
  ...
  const recarregarClinico = useCallback(async () => {
    const [{ data: est }, { data: planos }] = await Promise.all([
      ...
      supabase.rpc('fn_odonto_planos_paciente', { p_company_id: companyId, p_paciente_id: pacienteId }),
    ])
  ...
  useEffect(() => {
    let alive = true
    void (async () => { await Promise.all([carregarProcs(), recarregarClinico()]); if (alive) setCarregando(false) })()
    return () => { alive = false }
  }, [carregarProcs, recarregarClinico])

  // Vários lançamentos por dente/face são normais. A cor reflete o MAIS RELEVANTE: pendente (âmbar)
  // vence finalizado (verde). O contador `contagemDente` avisa quando há mais de um.
  ...
  // 💡 sugestão de preço vem do catálogo; se 0 → dentista define
  ...
  const salvarPlano = async (novos: Item[]) => {
    const payloadItens = novos.map((i, ix) => ({
      procedimento_id: i.procedimento_id, descricao: i.descricao, dente: i.dente, faces: i.faces, valor: i.valor, ordem: ix,
    }))
    const { data, error } = await supabase.rpc('fn_odonto_plano_salvar', {
      p_company_id: companyId,
      p_plano: { paciente_id: pacienteId, titulo: 'Plano clínico', status: 'orcamento' },
  ...
  const removerItem = async (ix: number) => {
    ...
    if (id) { setItens(novos); flash(true, 'Item removido.'); onMudou?.() }
  }
  ...
  if (carregando) return <div style={{ color: TOK.mut, fontSize: 13 }}>Carregando odontograma…</div>

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      {msg && <div style={{ background: msg.ok ? '#E7F3EA' : '#FBEBEB', color: msg.ok ? TOK.green : TOK.red, padding: '8px 12px', borderRadius: TOK.rCtrl, fontSize: 13, fontWeight: 600 }}>{msg.t}</div>}
      ...
      {aprovarAberto && planoId && (
        <AprovarModal planoId={planoId} total={total} onClose={() => setAprovarAberto(false)}
          onAprovado={(r) => { setAprovarAberto(false); flash(true, `Plano aprovado · ${r.titulos} título(s) · ${brl(r.valor)}`); void recarregarClinico(); onMudou?.() }} />
      )}
    </div>
  )
}
