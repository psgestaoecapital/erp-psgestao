// FEAT-PLUGGY-CONCILIACAO-FASE1-v1 · pluggy-connect-token-financeiro
// Variante de pluggy-connect-token pro contexto FINANCEIRO (conciliacao
// bancaria). Recebe company_id em vez de client_id (sem wealth_clients).
// Reusa /auth + /connect_token da Pluggy e o widget react-pluggy-connect.
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!body.company_id) return jsonError("company_id is required", 400);

    // Valida acesso do user a essa company via RLS (user_companies)
    const { data: comp, error: compErr } = await supabase
      .from("companies")
      .select("id, razao_social")
      .eq("id", body.company_id)
      .single();
    if (compErr || !comp) return jsonError("Company not found or no access", 403);

    // Consentimento: opcional na Fase 1 (sandbox), mas se vier no body, valida
    ...
    if (consentId) {
      const { data: consent } = await supabase
        ...
        .is("revogado_em", null)
        .single();
      if (!consent) return jsonError("consent_id invalido ou revogado", 400);
    }
    ...
    if (!pluggyClientId || !pluggyClientSecret) {
      return jsonError("Pluggy credentials not configured in edge env", 500);
    }
    ...
    const connectResp = await fetch(`${PLUGGY_API_URL}/connect_token`, {
      ...
      body: JSON.stringify({ options: { clientUserId: comp.id } }),
    });
    if (!connectResp.ok) {
    ...
    return new Response(
      JSON.stringify({
        ...
        company_id: comp.id,
        company_nome: comp.razao_social,
        consent_id: consentId,
      }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" }, status: 200 },
    );
  } catch (err) {
    console.error("pluggy-connect-token-financeiro error:", err);
