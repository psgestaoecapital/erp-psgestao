-- =============================================================
-- FEAT-NFE-PRODUTO-1-BASE-PEDIDO-v1 · NFe-1
-- =============================================================
-- Base da NF-e de produto a partir do pedido.
-- Espelha o pattern da NFS-e O3b (PR #284):
--  - Vinculo pedido <-> NF-e (erp_nfe_emitidas.pedido_id)
--  - RPC read-only consolidada (destinatario + itens com fiscal)
--  - RPC pra marcar emitida (vincula via provider_reference)
--
-- Migration aplicada via MCP em 2026-06-11.
-- =============================================================

ALTER TABLE erp_nfe_emitidas ADD COLUMN IF NOT EXISTS pedido_id uuid;
CREATE INDEX IF NOT EXISTS idx_nfe_emitidas_pedido ON erp_nfe_emitidas(pedido_id);
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'destinatario', jsonb_build_object(
        ...
        'municipio', v_cli.cidade, 'uf', v_cli.uf,
...
CREATE OR REPLACE FUNCTION public.fn_pedido_nfe_marcar_emitida(p_pedido_id uuid, p_provider_reference text)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $$
DECLARE v_id uuid;
BEGIN
  UPDATE erp_nfe_emitidas SET pedido_id = p_pedido_id
  WHERE provider_reference = p_provider_reference
    AND (pedido_id IS NULL OR pedido_id = p_pedido_id)
  RETURNING id INTO v_id;
  IF v_id IS NULL THEN RETURN jsonb_build_object('ok',false,'erro','NF-e nao encontrada'); END IF;
  RETURN jsonb_build_object('ok',true,'nfe_id',v_id);
END; $$;

GRANT EXECUTE ON FUNCTION public.fn_pedido_nfe_dados(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.fn_pedido_nfe_marcar_emitida(uuid,text) TO authenticated;
