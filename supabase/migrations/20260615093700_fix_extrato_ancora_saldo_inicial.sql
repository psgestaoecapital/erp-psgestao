-- =============================================================
-- fix_extrato_ancora_saldo_inicial · fn_ge_extrato_conta
-- =============================================================
-- Objetivo: o extrato passar a ancorar no saldo_inicial das contas
-- (erp_banco_contas), em vez de somar o historico de lancamentos do
-- zero. Mantem periodo, totais, filtros e LIMIT 500.
--
-- Logica:
--   v_saldo_base    = SUM(saldo_inicial) das contas ativas+soma_no_saldo
--                     (filtrado por conta se p_conta_id != NULL)
--   v_data_ancora   = MIN(data_saldo_inicial) das mesmas contas
--   v_c_inicio      = receber_pago(<= v_data_inicio - 1) - pagar_pago(<= ...)
--   v_c_ancora      = receber_pago(<= v_data_ancora)     - pagar_pago(<= ...)
--   v_saldo_anterior = v_saldo_base + v_c_inicio - v_c_ancora
--
-- Fallback legado: se v_data_ancora IS NULL (sem saldo_inicial cadastrado),
-- mantem comportamento antigo (soma historico do zero).
--
-- Resposta jsonb agora inclui em 'totalizadores': saldo_base + data_ancora
-- (visibilidade pro frontend).
--
