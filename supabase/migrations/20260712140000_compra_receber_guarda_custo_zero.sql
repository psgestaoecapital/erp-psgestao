-- ============================================================
-- MITIGAÇÃO BARATA: guarda custo_unitario > 0 em fn_compra_receber.
--
-- fn_compra_receber JÁ propaga o custo médio (média ponderada inline) e JÁ
-- preenche valor_total — NÃO é o bug do import. Mas faltava a guarda que a RPC
-- fn_movimentar_estoque tem: recompor o custo médio só quando o custo da
-- entrada > 0. Sem ela, uma compra com preço ZERO diluía o custo médio pra
-- baixo. Esta é a única mudança (1 ramo do CASE) — arquitetura intacta.
--
-- A refatoração completa (rotear pela RPC = autoridade única) fica registrada
-- como débito técnico em erp_contexto_projeto; não é bug vivo, é risco de drift.
-- fn_faturar (saída) permanece como está.
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    -- GUARDA custo-zero: recompoe o custo medio SO com custo > 0 (igual a RPC).
    -- Compra com preco 0 NAO dilui o custo medio; mantem o custo atual.
    v_novo_custo := CASE
      WHEN v_item.preco_unitario <= 0 THEN COALESCE(v_custo_atual, 0)
