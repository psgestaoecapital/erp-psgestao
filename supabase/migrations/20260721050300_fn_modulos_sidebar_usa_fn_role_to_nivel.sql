-- Refactor cirúrgico: fn_modulos_sidebar_por_area passa a usar fn_role_to_nivel (fonte única do role→nível).
-- ÚNICA mudança funcional é o CASE inline → chamada da função; o resto do corpo é idêntico ao vigente.
-- Provado por digest antes/depois (RD-38): saída byte-idêntica p/ Fabiane (industrial n=9, compliance n=21) e rh@ (n=2).
