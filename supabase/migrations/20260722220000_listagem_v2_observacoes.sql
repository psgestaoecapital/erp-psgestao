-- 4.3 — Expor `observacoes` na listagem de pagar/receber (o campo já existe nas tabelas).
-- fn_ge_listagem_v2 RETURNS jsonb (não RETURNS TABLE), então CREATE OR REPLACE é aditivo:
-- só acrescenta a coluna `observacoes` em cada SELECT de resultados. KPIs e demais campos
-- ficam byte-a-byte idênticos (prova de não-regressão por digest antes/depois).
  ...
  IF p_tipo = 'pagar' THEN
    ...
    FROM (
      SELECT id, descricao, fornecedor_nome AS nome_pessoa, categoria,
        ...
        status, numero_documento, forma_pagamento, observacoes,
