-- Provisionamento de acesso · Parte 2/3 — TODO caminho de escrita passa a completar user_scope (RD-57)
-- + fonte de LEITURA única (fn_acesso_efetivo). Assim "cadastrei acesso → telas reconhecem" vira verdade
-- automática, sem remendo por usuário. Patch aditivo (1 chamada ao helper) nas funções já existentes.

-- 1) Edição no admin: fn_acessos_salvar_pessoa agora também grava user_scope (via helper único).
...
BEGIN
  ...
  -- COMPLETA o provisionamento: user_scope (o que faltava). Não força em empresa sem org (helper decide).
...
-- 2) Vínculo direto via convite: fn_acessos_convidar_pessoa (ramo do usuário já existente) também completa user_scope.
CREATE OR REPLACE FUNCTION public.fn_acessos_convidar_pessoa(p_company_id uuid, p_email text, p_nome text DEFAULT NULL::text, p_areas text[] DEFAULT NULL::text[], p_role text DEFAULT 'viewer'::text, p_plantas uuid[] DEFAULT NULL::uuid[], p_horario jsonb DEFAULT NULL::jsonb, p_papel_gestao text DEFAULT 'CLIENT_VIEWER'::text)
 RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $function$
DECLARE
...
BEGIN
  ...
  IF v_uid IS NOT NULL THEN
    ...
    -- COMPLETA: user_scope no vínculo direto.
  ...
  ELSE
    ...
    RETURN jsonb_build_object('ok', true, 'acao','convidado', 'invite_id', v_invite_id, 'invite_code', v_code, 'link', '/convite?code='||v_code);
  END IF;
END; $function$;

-- 3) LEITURA ÚNICA: fn_acesso_efetivo — uma pergunta, uma resposta. Resolve TUDO que define o acesso de um
-- usuário numa empresa (papel operacional, papel de gestão, nível, rótulo, áreas, restrição, domínios, unidade).
-- É a fonte canônica para telas/sidebar (migração incremental das telas fica como próximo passo).
...
$$;
GRANT EXECUTE ON FUNCTION public.fn_acesso_efetivo(uuid, uuid) TO authenticated;
