-- Lote A.2 · aceita conta_bancaria_id/centro_custo_id no editor de Receber (aditivo; mantém os textos legados).
-- RLS do título + FK garantem escopo (SPEC permite confiar em RLS+FK).
...
BEGIN
  ...
  SELECT COALESCE(jsonb_object_agg(kk, jsonb_build_object('de', v_antes->kk, 'para', v_depois->kk)), '{}'::jsonb)
    INTO v_alterados FROM (SELECT jsonb_object_keys(v_tipos) AS kk) s
