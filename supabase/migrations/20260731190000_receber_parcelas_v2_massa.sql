-- RD-41 · Receitas a Receber — parcelas explícitas (data+valor por parcela) + ações em massa.
-- Espelho do #823 (pagar). Aditivo: fn_receber_criar_com_parcelas v1 preservada. As RPCs
-- em massa REUSAM as unitárias (fn_receber_editar_completo / fn_receber_excluir) → herdam
-- RLS (get_user_company_ids), autoria, soft-delete, bloqueios e log. RD-57.
-- Diferença do receber: fn_receber_excluir bloqueia também título com BOLETO ATIVO
-- (bloqueado_boleto_ativo) — a exclusão em massa reporta esse bloqueio à parte.

-- 1/3 · Criar recebimento com parcelas EXPLÍCITAS (permite valor/data variável)
CREATE OR REPLACE FUNCTION public.fn_receber_criar_com_parcelas_v2(
  p_company_id uuid, p_cliente_id uuid, p_cliente_nome text, p_descricao text,
  p_data_emissao date, p_categoria text DEFAULT NULL, p_numero_documento text DEFAULT NULL,
  p_forma_recebimento text DEFAULT NULL, p_observacao text DEFAULT NULL,
  p_conta_bancaria text DEFAULT NULL, p_status_inicial text DEFAULT 'aberto',
...
END; $function$;
GRANT EXECUTE ON FUNCTION public.fn_receber_criar_com_parcelas_v2(
  uuid,uuid,text,text,date,text,text,text,text,text,text,jsonb) TO authenticated;

-- 2/3 · Edição em massa (reusa fn_receber_editar_completo → herda RLS + log)
...
END; $function$;
GRANT EXECUTE ON FUNCTION public.fn_receber_editar_massa(uuid[], jsonb) TO authenticated;

-- 3/3 · Exclusão em massa (reusa fn_receber_excluir → soft-delete + bloqueios + log)
CREATE OR REPLACE FUNCTION public.fn_receber_excluir_massa(p_ids uuid[])
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public','pg_temp'
AS $function$
DECLARE v_id uuid; v_res jsonb; v_erro text;
        v_ok int := 0; v_bloq int := 0; v_boleto int := 0; v_ja int := 0; v_erros jsonb := '[]'::jsonb;
BEGIN
  IF p_ids IS NULL OR array_length(p_ids,1) IS NULL THEN
    RETURN jsonb_build_object('sucesso', false, 'erro', 'sem_ids'); END IF;
  FOREACH v_id IN ARRAY p_ids LOOP
    v_res := public.fn_receber_excluir(v_id);
    v_erro := v_res->>'erro';
    IF (v_res->>'sucesso')::boolean IS TRUE AND COALESCE((v_res->>'ja_excluido')::boolean,false)
      THEN v_ja := v_ja + 1;
    ELSIF (v_res->>'sucesso')::boolean IS TRUE
      THEN v_ok := v_ok + 1;
    ELSIF v_erro = 'bloqueado_conciliado_ou_pago' THEN v_bloq := v_bloq + 1;
    ELSIF v_erro = 'bloqueado_boleto_ativo'       THEN v_boleto := v_boleto + 1;
    ELSE v_erros := v_erros || jsonb_build_object('id', v_id, 'erro', v_erro);
    END IF;
  END LOOP;
  RETURN jsonb_build_object('sucesso', true,
    'excluidos', v_ok,
    'ignoradas_pago_conciliado', v_bloq,
    'ignoradas_boleto_ativo', v_boleto,
    'ja_excluidas', v_ja,
