-- RD-41 · PR2 — ETL honra de-para SÓ para investimento (fecha aceite #4 do #824).
-- Autorização CEO (RD-25: mexe em números do DRE). Escopo cirúrgico: apenas
-- tipo='investimento' sai do 6.11 operacional e vai pra 9.3 (NAO_OPER) via de-para.
-- NÃO é o remanejo geral do de-para (R$ 24M / 10k+ títulos) — esse fica fora de escopo.
--
-- Só os 2 blocos que hardcodavam '6.11' mudam: etl_pagar_caixa (regime caixa) e
-- etl_pagar_direto (regime competência, fallback). O resto da função é idêntico.
-- Para não-investimento, o psgc_codigo continua '6.11' exatamente como hoje (RD-53).
...
BEGIN
  ...
  v_tem_pagar_emissao := EXISTS(
    ...
    WHERE company_id=p_company_id AND data_emissao IS NOT NULL
      AND data_emissao BETWEEN v_data_inicio AND v_data_fim LIMIT 1
  );
  v_tem_receber_emissao := EXISTS(
    SELECT 1 FROM erp_receber
    WHERE company_id=p_company_id AND data_emissao IS NOT NULL
      AND data_emissao::date BETWEEN v_data_inicio AND v_data_fim LIMIT 1
  );
  ...
  v_tem_pagar_caixa := EXISTS(
    ...
    WHERE company_id=p_company_id AND data_pagamento IS NOT NULL
      AND data_pagamento BETWEEN v_data_inicio AND v_data_fim LIMIT 1
  );
  v_tem_receber_caixa := EXISTS(
    SELECT 1 FROM erp_receber
    WHERE company_id=p_company_id AND data_pagamento IS NOT NULL
      AND data_pagamento::date BETWEEN v_data_inicio AND v_data_fim LIMIT 1
  );
  ...
  IF v_tem_omie THEN
    ...
    FROM (
      ...
      WHERE r.company_id=p_company_id AND r.data_emissao IS NOT NULL
        AND r.data_emissao::date BETWEEN v_data_inicio AND v_data_fim
  ...
  -- RD-41 PR2: investimento -> 9.3 (via de-para NAO_OPER); resto continua '6.11'.
  IF v_tem_pagar_emissao AND NOT v_tem_lancamentos_no_mes AND NOT v_tem_omie THEN
    ...
    FROM (
      ...
      WHERE p.company_id = p_company_id AND p.data_emissao IS NOT NULL
        AND p.data_emissao BETWEEN v_data_inicio AND v_data_fim
  ...
  -- ║   NOVO 15/05/2026 - Foundational       ║
  -- ╚════════════════════════════════════════╝

  -- Despesas pagas no mes (regime caixa) - sempre lê de erp_pagar direto
  -- pois caixa é universal (data_pagamento independe de Omie/lancamento)
  -- RD-41 PR2: investimento -> 9.3 (via de-para NAO_OPER); resto continua '6.11'.
  ...
  -- Receitas recebidas no mes (regime caixa)
  IF v_tem_receber_caixa THEN
    ...
    FROM erp_receber r WHERE r.company_id=p_company_id AND r.data_pagamento IS NOT NULL
      AND r.data_pagamento::date BETWEEN v_data_inicio AND v_data_fim
      AND (r.status IS NULL OR r.status != 'cancelado')
    HAVING COUNT(*)>0
    ON CONFLICT (company_id, ano, mes, ln_id, psgc_codigo, regime, origem_sistema_lancamento, source) DO UPDATE
      SET valor=EXCLUDED.valor, qtd_lancamentos=EXCLUDED.qtd_lancamentos, calculated_at=NOW();
