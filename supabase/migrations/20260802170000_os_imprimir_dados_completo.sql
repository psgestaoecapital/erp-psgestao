-- RD-41 · Oficina 🅱️ — impressão/orçamento completo da OS. Completa a MESMA RPC (RD-26).
-- Faltava: placa/veículo/KM, itens do DIAGNÓSTICO (peças+serviços com preco/aprovado — #842),
-- recusados SEM valor (honesto RD-51), resumo (total aprovado), ramo (retífica sem automotivo),
-- e fotos (opcional, PDF leve por padrão). FRONTEIRA GE: documento operacional; a NF fiscal é [→GE].
-- Também adiciona o GUARD de acesso que faltava (era SECURITY DEFINER sem escopo de empresa).

-- dropa a arity antiga (1 arg) p/ o default do novo p_incluir_fotos não ficar ambíguo
DROP FUNCTION IF EXISTS public.fn_os_imprimir_dados(uuid);
  ...
  RETURN jsonb_build_object(
    ...
    'fotos', CASE WHEN p_incluir_fotos THEN COALESCE((
        SELECT jsonb_agg(jsonb_build_object(
          ...
          'autor', f.criado_por_nome, 'data', f.created_at) ORDER BY f.created_at)
...
GRANT EXECUTE ON FUNCTION public.fn_os_imprimir_dados(uuid, boolean) TO authenticated;
