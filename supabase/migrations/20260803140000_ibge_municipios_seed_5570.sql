-- RD-41 · Tabela completa de municípios IBGE (5.570) — seed idempotente.
-- CEO (03/08): código IBGE é obrigatório na NFS-e. RD-26: popula a tabela EXISTENTE
-- (erp_gov_nfse_municipios), não cria nova. RD-42: fonte oficial IBGE, grátis, seed único.
-- Idempotente (upsert por codigo_ibge): re-rodar não duplica; corrige nome/uf pré-existentes
-- e PRESERVA aderido/data_adesao/observacoes. Formato dollar-quoted (blob pipe+quebra-de-linha)
-- p/ evitar escape de apóstrofo. Fonte verificada: 5.570 códigos únicos; contagens por UF
-- conferem (SC=295, SP=645, MG=853; 27 UFs); 5 municípios mais novos presentes.
-- NOTA (RD-38): São Miguel do Oeste = 4217204 (o 4217303 citado no SPEC é Saudades/SC).

INSERT INTO public.erp_gov_nfse_municipios (codigo_ibge, nome_municipio, uf, atualizado_em)
SELECT split_part(x, '|', 1), split_part(x, '|', 2), split_part(x, '|', 3), now()
...
2403301|Encanto|RN
2403400|Equador|RN
2403509|Espírito Santo|RN$blob$, E'\n')) AS x
WHERE x <> ''
ON CONFLICT (codigo_ibge) DO UPDATE SET
  nome_municipio = EXCLUDED.nome_municipio, uf = EXCLUDED.uf, atualizado_em = now();
...
3506409|Bilac|SP
3506508|Birigui|SP
3506607|Biritiba-Mirim|SP$blob$, E'\n')) AS x
WHERE x <> ''
ON CONFLICT (codigo_ibge) DO UPDATE SET
  nome_municipio = EXCLUDED.nome_municipio, uf = EXCLUDED.uf, atualizado_em = now();
...
4209607|Lauro Muller|SC$blob$, E'\n')) AS x
WHERE x <> ''
ON CONFLICT (codigo_ibge) DO UPDATE SET
  nome_municipio = EXCLUDED.nome_municipio, uf = EXCLUDED.uf, atualizado_em = now();
