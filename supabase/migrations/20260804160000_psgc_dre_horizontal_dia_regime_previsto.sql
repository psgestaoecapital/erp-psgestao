-- RD-41 · DRE Consolidado: +1 regime 'previsto' (fluxo por VENCIMENTO). Pilar 1: competência/caixa
-- INTOCADOS (mesmo código); 'previsto' é ramo próprio com RETURN antecipado — NÃO passa pela máquina
-- do DRE (nada de EBITDA/margem/lucro; previsão é fluxo de caixa, não resultado). RD-26: reusa
-- fn_saldo_bancos_dinamico p/ o saldo inicial. RD-51: dia sem movimento = 0 real (nada inventado).
...
DECLARE
  ...
  v_dias     jsonb;
  v_linhas   jsonb;
  v_saldo_ini numeric := 0;   -- previsto: saldo atual dos bancos do grupo (RD-26)
  v_dia_neg   text;           -- previsto: 1o dia (ymd) em que o saldo acumulado fica negativo
BEGIN
  ...
  -- REGIME 'previsto' (fluxo por vencimento) - ramo isolado, nao toca no DRE
  IF v_regime = 'previsto' THEN
    ...
    SELECT count(*) INTO v_empresas FROM (
      SELECT company_id FROM erp_receber
       WHERE company_id = ANY(p_company_ids) AND data_vencimento IS NOT NULL
         AND lower(COALESCE(status,'')) NOT IN ('pago','recebido','cancelado')
         AND data_vencimento::date BETWEEN v_ini AND v_fim
      UNION
      SELECT company_id FROM erp_pagar
       WHERE company_id = ANY(p_company_ids) AND data_vencimento IS NOT NULL
         AND lower(COALESCE(status,'')) NOT IN ('pago','cancelado')
         AND data_vencimento::date BETWEEN v_ini AND v_fim
    ) e;

    WITH ent AS (  -- entradas previstas: receber a vencer, pela data_vencimento
      SELECT to_char(r.data_vencimento::date,'YYYY-MM-DD') AS ymd, SUM(COALESCE(r.valor,0)) AS v
        FROM erp_receber r
    ...
    sai AS (  -- saidas previstas: pagar a vencer, pela data_vencimento
      SELECT to_char(p.data_vencimento::date,'YYYY-MM-DD') AS ymd, SUM(COALESCE(p.valor,0)) AS v
        FROM erp_pagar p
    ...
    dd AS (
      SELECT to_char(make_date(p_ano,p_mes,d),'YYYY-MM-DD') AS ymd, d
        FROM generate_series(1, extract(day FROM v_fim)::int) d
    ),
    fluxo AS (
      SELECT dd.ymd, dd.d,
             COALESCE(e.v,0) AS entradas, COALESCE(s.v,0) AS saidas,
             COALESCE(e.v,0) - COALESCE(s.v,0) AS movimento
        FROM dd LEFT JOIN ent e ON e.ymd = dd.ymd LEFT JOIN sai s ON s.ymd = dd.ymd
    ),
    acc AS (
      SELECT ymd, d, entradas, saidas, movimento,
             v_saldo_ini + SUM(movimento) OVER (ORDER BY d) AS saldo_acum
        FROM fluxo
    )
    SELECT
      jsonb_build_array(
        jsonb_build_object('ordem',1,'kind','fluxo','codigo','ENTRADAS','nome','Entradas previstas','sinal','+','nivel',0,'colapsavel',false,
          'valores_dia', (SELECT jsonb_object_agg(ymd, round(entradas,2)) FROM acc)),
        jsonb_build_object('ordem',2,'kind','fluxo','codigo','SAIDAS','nome','Saídas previstas','sinal','-','nivel',0,'colapsavel',false,
          'valores_dia', (SELECT jsonb_object_agg(ymd, round(saidas,2)) FROM acc)),
        jsonb_build_object('ordem',3,'kind','fluxo','codigo','MOVIMENTO','nome','Movimento do dia','sinal','±','nivel',0,'colapsavel',false,
          'valores_dia', (SELECT jsonb_object_agg(ymd, round(movimento,2)) FROM acc)),
        jsonb_build_object('ordem',4,'kind','fluxo','codigo','SALDO','nome','Saldo acumulado projetado','sinal','=','nivel',0,'colapsavel',false,
          'valores_dia', (SELECT jsonb_object_agg(ymd, round(saldo_acum,2)) FROM acc))
      ),
      (SELECT min(ymd) FROM acc WHERE saldo_acum < 0)
      INTO v_linhas, v_dia_neg;
