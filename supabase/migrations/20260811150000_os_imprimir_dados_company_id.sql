-- OS "via operacional" (impressão sem valores) — Fase A.
-- Expõe company_id no retorno de fn_os_imprimir_dados p/ a tela de impressão resolver
-- o papel do usuário na empresa (useAcesso -> fn_acesso_efetivo -> tenant_user_roles) e
-- forçar o modo sem valores quando CLIENT_OPERATOR. Reusa o hook canônico (RD-26): não
-- criamos fn_meu_papel_empresa. Aditivo: só acrescenta um campo ao JSON já retornado.
