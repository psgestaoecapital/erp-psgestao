-- RD-41 · Fase 1 (Saneamento) — Filtro de Status em múltipla seleção (a pagar + a receber).
-- Origem: Jordana KGF — poder combinar Vencidos + Pagas numa única pesquisa.
-- Adiciona p_status_filtros text[] (opcional, ao final): quando não-nulo/não-vazio, filtra a LISTAGEM
-- por um OR dos predicados dos status marcados (avencer/vencidos/hoje/pagos). NULL/vazio → cai no
-- p_status_filtro (single) atual = 'todos' → sem regressão. Os KPIs seguem calculando todos os baldes.
-- Assinatura muda (novo param) → DROP + CREATE; chamadas atuais (5 args named) seguem válidas.

DROP FUNCTION IF EXISTS public.fn_ge_listagem_v2(uuid, text, date, date, text);

CREATE OR REPLACE FUNCTION public.fn_ge_listagem_v2(
  p_company_id uuid, p_tipo text, p_data_inicio date, p_data_fim date,
  p_status_filtro text DEFAULT 'todos'::text,
      ...
      WHERE d.company_id = p_company_id AND d.deleted_at IS NULL AND d.data_vencimento BETWEEN p_data_inicio AND p_data_fim
        AND (
          CASE WHEN v_multi THEN (
              ...
              OR ('vencidos' = ANY(p_status_filtros) AND d.data_vencimento < v_hoje AND d.status != 'pago')
              OR ('hoje'     = ANY(p_status_filtros) AND d.data_vencimento = v_hoje AND d.status != 'pago')
              OR ('avencer'  = ANY(p_status_filtros) AND d.data_vencimento > v_hoje AND d.status != 'pago')
...
GRANT EXECUTE ON FUNCTION public.fn_ge_listagem_v2(uuid, text, date, date, text, text[]) TO authenticated;
