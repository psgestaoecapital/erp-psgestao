-- Orçamento · alíquota ISS editável por item (override do cadastro). Fronteira GE (fiscal/comercial).
--
-- A R.R muda a alíquota mensalmente → mais fácil preencher no faturamento do que editar o cadastro do
-- serviço toda vez. Adiciona o override por item; o default vem de erp_servicos.aliquota_iss.
-- RD-51: override explícito, null = usa o do cadastro (coalesce). Não toca o cadastro do serviço.

ALTER TABLE public.erp_orcamentos_itens ADD COLUMN IF NOT EXISTS aliquota_iss numeric;
COMMENT ON COLUMN public.erp_orcamentos_itens.aliquota_iss IS
  'Override da alíquota ISS deste item. Null = usa o do cadastro (erp_servicos.aliquota_iss).';

-- A RPC de salvar itens (edições não-rascunho) tem lista explícita de colunas → precisa carregar o novo campo.
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_itens) LOOP
    ...
    INSERT INTO erp_orcamentos_itens(
      ...
      margem_percentual,bdi_percentual,aliquota_iss)
    VALUES(
      ...
      NULLIF(v_item->>'aliquota_iss','')::numeric);
    v_count:=v_count+1;
  END LOOP;
