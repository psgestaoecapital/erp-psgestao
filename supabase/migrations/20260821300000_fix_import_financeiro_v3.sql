-- Fix da fn_import_financeiro_v3 — dois bugs que impediam QUALQUER importação com ref_externa_id
-- (inclui a tela de Importação Universal). Descobertos ao auditar a reimportação da Estância Umuarama.
--
-- BUG 1 (crítico): o INSERT usa `ON CONFLICT (company_id, ref_externa_sistema, ref_externa_id) DO NOTHING`,
--   mas o índice único é PARCIAL: uq_erp_pagar_ref_externa ... WHERE ref_externa_id IS NOT NULL.
--   Postgres exige que o ON CONFLICT replique o predicado do índice parcial; sem ele, cada linha falha com
--   "there is no unique or exclusion constraint matching the ON CONFLICT specification" → 0 inseridos.
--   Correção: adicionar `WHERE ref_externa_id IS NOT NULL` ao ON CONFLICT (pagar e receber).
--
-- BUG 2: a função faz jsonb_array_length(p_records) — espera um ARRAY puro. Payloads no formato
--   {"records": [...]} (o padrão exportado) quebram com "cannot get array length of a non-array".
--   Correção: se vier objeto com a chave "records", desembrulha para o array.
--
-- RD-52: comportamento idêntico ao anterior fora esses dois pontos.
