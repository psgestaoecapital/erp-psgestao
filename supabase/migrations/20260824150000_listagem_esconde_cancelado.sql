-- FIX (financeiro) · Receitas a Receber mostrava títulos CANCELADOS como vencido/a-vencer.
--
-- Auditoria (RD-38): fn_ge_listagem_v2 (ramo receber) não excluía status 'cancelado' → 30 títulos
-- cancelados vazavam para a listagem e somavam nos KPIs (vencidos/a-vencer), inflando o recebível.
-- Bug pré-existente, distinto da renegociação (RENEG-F1 já escondeu 'renegociado'). Aqui estendo a
-- mesma exclusão para 'cancelado' no ramo receber (KPIs + resultados). Ramo 'pagar' intacto.
--
-- Estado final da função = esconde 'orcamento' (já existia) + 'renegociado' (RENEG-F1) + 'cancelado'.
-- Timestamp posterior ao da RENEG-F1 (…140000) para ser o último a aplicar. Merge após a #1117.
