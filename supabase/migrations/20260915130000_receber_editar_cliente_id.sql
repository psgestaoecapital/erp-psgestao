-- ============================================================
-- #71 (Jordana) · editar recebível deve poder TROCAR o cliente (cliente_id), não só o nome
-- ============================================================
-- Sintoma: "alterei o cliente e o sistema não confirma". Causa provada: a tela de edição só
-- gravava cliente_nome (texto) e a allowlist de fn_receber_editar_completo NÃO tinha cliente_id —
-- então o VÍNCULO nunca mudava. O boleto/NF lê cliente_id → continuava usando o cliente antigo
-- (2º fator do bug do Sicredi #1462). Aqui: acrescenta 'cliente_id','uuid' à allowlist. Nada mais
-- muda no corpo. Provado em rollback impersonando a autora (cliente_id de/para gravou; log EDITOU).
...
DECLARE
  v_tipos jsonb := jsonb_build_object(
    'cliente_id','uuid','cliente_nome','text','descricao','text','categoria','text','valor','numeric',
