-- #82② cluster obra · Parte 3 (elo faltante) — a conversão orçamento→pedido tem que CARREGAR a obra.
--
-- A cadeia do Rodrigo é: orçamento (informa a obra na tela) → CONVERTE → pedido → emite NFS-e.
-- A emissão lê a obra do PEDIDO (fn_pedido_nfse_dados, Parte 2). Mas fn_converter_orcamento_em_pedido
-- listava colunas explícitas no INSERT e NÃO incluía os campos obra_* — então a obra informada no
-- orçamento se perdia na conversão e o pedido nascia sem obra (obra_pendente=true, E0370 falha).
-- Este CREATE OR REPLACE só ACRESCENTA os 10 campos obra_* ao INSERT (colunas + values a partir do
-- v_orc). Todo o resto é idêntico byte-a-byte. RD-30/RD-53.
