-- Adendo #1541 "Saldo Anterior" — origem do saldo vinda do arquivo (OFX line vs LEDGERBAL).
-- Alguns bancos (ex.: Sicredi/Sicoob) não mandam <LEDGERBAL>; emitem o saldo como uma LINHA
-- <STMTTRN> com MEMO "SALDO ANTERIOR"/"SALDO DO DIA"/só "SALDO". O front (importarOFX) já:
--   (a) NÃO importa essas linhas como movimento; (b) quando não há LEDGERBAL, usa a linha de
--   saldo mais recente como fallback e manda p_saldo_bruto->>'origem' = 'ofx_saldo_linha'.
-- Aqui o único ajuste no backend: a origem gravada em erp_banco_contas.saldo_extrato deixa de ser
-- o literal 'ofx' e passa a respeitar p_saldo_bruto->>'origem' (default 'ofx' — LEDGERBAL). Ponto
-- único de escrita permanece fn_banco_saldo_registrar (RD-65). Assinatura idêntica (16 args) →
-- CREATE OR REPLACE preserva os GRANTs de #1541 (authenticated, service_role).
...
BEGIN
  ...
  -- Saldo de fechamento: PONTO ÚNICO de escrita (RD-65), na MESMA transação. Independe de haver
  -- movimentos novos — o snapshot do saldo vale de qualquer forma. fn_banco_saldo_registrar nunca
  -- retrocede (RD-67): leitura mais antiga → gravado=false. A ORIGEM vem do arquivo (LEDGERBAL →
  -- 'ofx'; linha "SALDO ANTERIOR/DO DIA" → 'ofx_saldo_linha'), preservando a rastreabilidade.
  IF p_saldo_fechamento IS NOT NULL AND p_saldo_data IS NOT NULL AND p_conta_bancaria_id IS NOT NULL THEN
    v_saldo_res := fn_banco_saldo_registrar(
      p_conta_bancaria_id, p_saldo_fechamento,
      COALESCE(NULLIF(p_saldo_bruto->>'origem',''), 'ofx'),
      jsonb_build_object('balamt_bruto', p_saldo_bruto->>'balamt_bruto',
                         'dtasof_bruto', p_saldo_bruto->>'dtasof_bruto',
                         'descricao_linha', p_saldo_bruto->>'descricao_linha',
...
-- Assinatura idêntica a #1541 → grants preservados. Reafirma por segurança (idempotente).
