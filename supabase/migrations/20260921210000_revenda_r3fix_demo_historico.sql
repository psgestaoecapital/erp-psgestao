-- Revenda R3-fix (dados) · Histórico da Demo: eventos + linha do tempo do valor
--
-- POR QUÊ (RD-38 · prova no dado): a auditoria R3 apontou T4 (Ficha) abaixo da meta porque a seção
-- "Histórico de alterações" aparecia com "Sem eventos" e a "Linha do tempo do valor" vazia. A prova no
-- banco confirmou: a Demo Revenda ([BOT] Revenda) tinha 13 veículos, mas 0 linhas em veic_veiculo_evento
-- e 0 em veic_precificacao_hist. A tela estava correta (estado vazio honesto) — faltava o DADO real do ciclo.
--
-- O CEO deu a escolha explícita: "ou alimentar com os eventos reais (audit_log do veículo + histórico de
-- preço), ou a seção não deveria estar na tela." Aqui seguimos a via de ALIMENTAR — com o histórico
-- verdadeiro do ciclo já modelado no seed (entrada, precificação, custos, mudança de situação, devolução).
--
-- ONDE (RD-69 · seed determinístico por vertical): o histórico nasce dentro de fn_gold_revenda_seed_reparar,
-- que é o seed que o fn_demo_reset chama para a Revenda. Assim o histórico SOBREVIVE a resets da demo em vez
-- de sumir no próximo reset. É estritamente aditivo e guardado por company_id = [BOT] Revenda (empresa demo).
--
-- RD-51 (nunca fingir): os valores da linha do tempo derivam do próprio veículo (aquisição, piso e anunciado
-- já semeados). Nada é inventado fora do que o ciclo do carro conta. O veículo "parado há 150 dias" mostra o
-- corte de preço real (anunciou caro, baixou para girar) — a história que o dono precisa enxergar.
