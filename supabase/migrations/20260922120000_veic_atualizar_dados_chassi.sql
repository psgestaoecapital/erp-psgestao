-- Revenda · R7b-2 (Completar dados) — permitir editar o CHASSI em lista e fechar a função ao anon.
--
-- fn_veic_atualizar_dados já aceita tipo/placa/renavam/cor/portas etc., mas NÃO o chassi (às vezes digitado
-- errado na entrada). Adiciona o chassi (mesma validação leve dos demais: só grava quando a chave vem e
-- não-vazia). A guarda de empresa (get_user_company_ids()/is_admin()) já existe — mantida. De quebra,
-- REVOKE do anon (a função é SECURITY DEFINER; nada a fazer para não-sessão).
...
BEGIN
  ...
  UPDATE veic_veiculo SET
    ...
    -- R7b-2: chassi editável (correção de digitação na entrada).
