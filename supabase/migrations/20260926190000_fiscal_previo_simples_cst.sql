-- NF-e OS-0179 (KGF · contexto d869704f). Pré-voo fiscal ganha o predicado "empresa do Simples com produto
-- em CST de regime normal (2 dígitos: 00, 10, 20… 90) em vez de CSOSN (3 dígitos: 101…900)". É o MESMO
-- predicado que o nfe-validator passou a travar antes do envio (RD-65: mesma régua na tela e na emissão) —
-- sem ele a Focus recusa a nota na hora por schema (CST não cabe no grupo ICMSSN) e o operador não sabe por quê.
-- Simples = companies.regime_tributario contém 'simples' (mesma fonte e teste do nfe-builder: ehSimples).
-- Prova 26/09: 3 produtos ativos da KGF com cst_icms='00' — os da OS-0179.
--
-- ADITIVO: corpo reproduzido da versão viva (20260924140000); só entram a contagem nova
-- produtos.simples_cst_regime_normal, o motivo na amostra e a soma em incompletos/produtos_incompletos.
-- Única mudança de comportamento: a ORDEM da amostra (15 itens) — primeiro o que causa recusa, por último
-- o "sem tipo do item (SPED)". Nenhuma chave existente muda de nome ou de tipo.
-- SECURITY DEFINER → REVOKE anon + GRANT (gate).
...
begin
  ...
  into v_sem_ncm, v_cst_st, v_ncm2710, v_sem_sped, v_simples_cst
  ...
  select coalesce(jsonb_agg(x), '[]'::jsonb) into v_prod_amostra from (
    ...
    order by (case
                ...
                when regexp_replace(coalesce(ncm,''),'\D','','g') !~ '^\d{8}$'
                  or (cst_icms in ('500','60') and (vbcst_ret is null or pst is null or vicms_substituto is null or vicms_st_ret is null))
                  or (regexp_replace(coalesce(ncm,''),'\D','','g') like '2710%' and (combustivel_codigo_anp is null or coalesce(btrim(combustivel_descricao_anp),'')='')) then 1
                else 2 end),
             codigo
    limit 15
  ) x;
  ...
  return jsonb_build_object(
    ...
    'produtos', jsonb_build_object(
      'incompletos', coalesce(v_sem_ncm,0)+coalesce(v_cst_st,0)+coalesce(v_ncm2710,0)+coalesce(v_sem_sped,0)+coalesce(v_simples_cst,0),
    ...
    'resumo', jsonb_build_object(
      ...
      'produtos_incompletos', coalesce(v_sem_ncm,0)+coalesce(v_cst_st,0)+coalesce(v_ncm2710,0)+coalesce(v_sem_sped,0)+coalesce(v_simples_cst,0),
