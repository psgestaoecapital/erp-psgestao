-- Pré-voo fiscal · formato da amostra de produtos (achado na prova pós-deploy do #1820, 26/09).
-- Desde 20260924020000 a amostra era jsonb_agg(x) de uma subquery com coluna SEM nome → cada item saía
-- embrulhado como {"jsonb_build_object": {codigo, nome, motivos}}. O PreVooFiscalCard lê p.codigo/p.motivos
-- direto: p.motivos era undefined e p.motivos.length derrubava o card em toda empresa com produto incompleto
-- (KGF: 1.764). Prova no dado (26/09 12:20): top-3 da KGF vinham como {"jsonb_build_object": {...}}.
-- Conserto na FONTE (RD-65): jsonb_agg(x.o) com a coluna nomeada → cada item é {codigo, nome, motivos}, o
-- contrato que o card e a spec de aceitação já esperam. Destinatários e serviços já agregavam o objeto direto.
-- Corpo idêntico ao da 20260926190000 fora essas 2 linhas. SECURITY DEFINER → REVOKE anon + GRANT (gate).
